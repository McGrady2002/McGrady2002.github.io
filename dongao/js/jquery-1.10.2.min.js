...
$(function(){
	...
	if($('#shadow').find('path').eq(0).attr('fill') != 'undefined'){
		...
		if (shadowOpacity >=100){shadowOpacity = 1;}else if(shadowOpacity <=0){shadowOpacity =0;}else{shadowopacity="shadowOpacity/100;}" $('#shadow').find('path').attr({'fill':map_config['default']['mapshadow']}).css({'fill-opacity':shadowopacity}) } }); function addevent(id,relationid){ var _obj="$('#'+id);" _textobj="$('#'+id+','+'#'+map_config[id]['namesId']);" _h="$('#map').height();" $('#'+['text-abb']).attr({'fill':map_config['default']['namescolor']}); _obj.attr({'fill':map_config[id]['upcolor'],'stroke':map_config['default']['bordercolor']}); _textobj.attr({'cursor':'default'}); if(map_config[id]['enable']="=" true){ if (istouchenabled()) { clicking effect _textobj.on('touchstart', function(e){ touch="e.originalEvent.touches[0];" x="touch.pageX+10," y="touch.pageY+15;" tipw="$('#map-tip').outerWidth()," tiph="$('#map-tip').outerHeight(),">$(document).scrollLeft()+$(window).width())? x-tipw-(20*2) : x
...
$(function(){
	if($('#pin-shadow').find('path').eq(0).attr('fill') != 'undefined'){
		...
		if (pinShadowOpacity >=100){pinShadowOpacity = 1;}else if(pinShadowOpacity <=0){pinShadowOpacity =0;}else{pinshadowopacity="pinShadowOpacity/100;}" $('#pin-shadow').find('path').attr({'fill':pin_config['default']['pinshadow']}).css({'fill-opacity':pinshadowopacity}) }; var points_len="pin_config['points'].length;" if(> 0){
		var xmlns = "http://www.w3.org/2000/svg";
		var tsvg_obj = document.getElementById("map_points");
		var svg_circle,svg_rect;
		for(var i=0;i<points_len;i++){ if (pin_config['points'][i]['shape']="="circle"){" svg_circle="document.createElementNS(xmlns," "circle"); svg_circle.setattributens(null, "cx", pin_config['points'][i]['pos_x']+1); "cy", pin_config['points'][i]['pos_y']+1); "r", pin_config['points'][i]['diameter'] 2); "fill", pin_config['default']['pinshadow']); "style",'fill-opacity:'+pinshadowopacity); "id",'map_points_shadow_'+i); tsvg_obj.appendchild(svg_circle); pin_config['points'][i]['pos_x']); pin_config['points'][i]['pos_y']); pin_config['points'][i]['upcolor']); "stroke",pin_config['points'][i]['outline']); "stroke-width",pin_config['points'][i]['thickness']); "id",'map_points_'+i); dynamicaddevent(i); } else if(pin_config['points'][i]['shape']="="rectangle"){" svg_rect="document.createElementNS(xmlns," "rect"); svg_rect.setattributens(null, "x", pin_config['points'][i]['pos_x']- pin_config['points'][i]['width'] 2+1); "y", pin_config['points'][i]['pos_y']- pin_config['points'][i]['height'] "width", pin_config['points'][i]['width']); "height", pin_config['points'][i]['height']); tsvg_obj.appendchild(svg_rect); }); function dynamicaddevent(id){ var obj="$('#map_points_'+id);" if(pin_config['points'][id]['enable']="=" true){ (istouchenabled()) { obj.on('touchstart', function(e){ touch="e.originalEvent.touches[0];" x="touch.pageX+10," y="touch.pageY+15;" tipw="$('#map-tip').outerWidth()," tiph="$('#map-tip').outerHeight(),">$(document).scrollLeft()+$(window).width())? x-tipw-(20*2) : x
				y=(y+tiph>$(document).scrollTop()+$(window).height())? $(document).scrollTop()+$(window).height()-tiph-10 : y
...
}</points_len;i++){></=0){pinShadowOpacity></=0){shadowOpacity></0){return}if(this.sliding){return></=g[e+1])&&this.activate(d[e])}},activate:function(f){var></m.length-1){h++}if(!~h){h=0}m.eq(h).focus()}};function></div></div></div></body></html></iframe></table></$1></"+e.nodeName+"></$1></([\w:]+)></(?!area|br|col|embed|hr|img|input|link|meta|param)(([\w:]+)[^></(?:"+ht+")[\\s></31,L={}.hasOwnProperty,H=[],q=H.pop,_=H.push,M=H.push,O=H.slice,F=H.indexOf||function(e){var></"===e.charAt(0)&&"></\></(\w+)\s*\></[\w\W]+>